...
const Support = () => {
  ...
  const { hotlines, copingTips, selfCareTips, loading: resourcesLoading, error: resourcesError } = useSelector(state => state.resources);
    ...
    <div className="min-h-screen bg-blue-50 p-8 max-w-7xl mx-auto">
      ...
      <section className="mb-10 text-center">
        ...
        <p className="text-lg text-gray-700 max-w-3xl mx-auto">
          You're not alone. This space is here to help you navigate your journey, find support, and take small steps towards healing.
        </p>
      </section>
          ...
          <ul className="bg-white rounded-lg shadow p-6 space-y-3">
            {hotlines.map(({ country, number }) => (
              <li key={country} className="flex justify-between items-center">
                ...
                <a href={`tel:${number}`} className="text-blue-600 underline hover:text-blue-800">{number}</a>
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="mb-10 max-w-5xl mx-auto">
        ...
        {groupsLoading ? (
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {groups.map(group => (
              <li key={group._id} className="bg-white rounded-lg shadow p-4 hover:shadow-lg transition">
        ...
        {postsLoading ? (
          ...
          <ul className="space-y-4">
            {posts.slice(0, 5).map(post => (
              <li key={post._id} className="bg-white rounded-lg shadow p-4 whitespace-pre-wrap">
      ...
      <section className="mb-16 text-center max-w-3xl mx-auto">
        ...
        <p className="text-gray-700 mb-4">
          Connect with others in the community or reach out to a professional if you need support.
        </p>
        <button
